...
foldchange_df <-function(x.data){
  ...
  this_info <- info_sample %>% 
    dplyr::mutate(block_start = ifelse(block==1, 0, 28))
  ...
  for(i in 1:nrow(this_info)){
    if(this_info$plot_timepoint[i] %in% my_index$plot_timepoint){
      use_rows = which(my_index$plot_timepoint == this_info$plot_timepoint[i])
      baseline_rows = which(my_index$plot_timepoint == this_info$block_start[i])
...
foldchange_list <- function(x.list){
  out=sapply(names(x.list), function(x){
    ...
    my_code=info_sample %>% dplyr::filter(timepoint %in% my_tp) %>% 
      dplyr::mutate(block_start = ifelse(block==1, 1, 21))
    this_out=data.frame()
    for(i in 1:nrow(my_code)){
        this_tp = my_data[which(my_tp == my_code$timepoint[i]),] %>% log2()
        this_base = my_data[which(my_tp == info_sample$timepoint[which(info_sample$timepoint==my_code$block_start[i])]),] %>% log2()
...
for(ii in files_list){
  ...
  saveRDS(out, file = paste0(wd,"/data/",gsub(pattern="_raw",replacement="_fcblock",x=ii)))
}
